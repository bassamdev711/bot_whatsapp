...
const emptyAssistantSettings: AssistantSettings = { enabled: false, ownerName: "بسّام", strictness: "strict", allowActivityContext: false, customInstructions: "", updatedAt: "" };
...
export default function Home() {
  ...
  const [assistantSettings, setAssistantSettings] = useState<AssistantSettings>(emptyAssistantSettings);
  ...
  useEffect(() => {
    ...
    const loadAssistantSettings = async () => {
      try {
        const response = await fetch("/api/assistant/settings", { cache: "no-store" });
        if (response.ok && active) setAssistantSettings(await response.json() as AssistantSettings);
      } catch {
        if (active) setAssistantError("تعذر تحميل إعدادات المساعد.");
      }
    };
  ...
  const selectNav = (label: string) => {
    ...
    if (label === "مساعد بسّام") setShowAssistantPanel(true);
    if (label === "الإعدادات") setShowSettingsPanel(true);
  };

  const persistAssistantSettings = async (nextSettings: AssistantSettings) => {
    setAssistantBusy(true);
    setAssistantError(null);
    try {
      const response = await fetch("/api/assistant/settings", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(nextSettings) });
      const payload = await response.json() as AssistantSettings & { error?: string };
      if (!response.ok) throw new Error(payload.error || "تعذر حفظ إعدادات المساعد.");
      setAssistantSettings(payload);
    } catch (error) {
      setAssistantError(error instanceof Error ? error.message : "تعذر حفظ إعدادات المساعد.");
    } finally {
  ...
  return (
    <div className="wasla-luxury min-h-screen bg-[#f3f2ed] text-[#15362f]" dir="rtl">
        ...
        <div className="mx-auto max-w-[1440px] px-5 py-7 sm:px-8 sm:py-9 lg:px-10">
          <section className="luxury-hero relative overflow-hidden rounded-[28px] border border-[#d6dfd8] bg-[#edf5ef] p-6 shadow-[0_20px_55px_rgba(46,83,67,0.08)] sm:p-8 lg:min-h-[298px] lg:p-10">
            ...
            <div className="relative z-10 max-w-xl">
              ...
              <div className="mt-7 flex flex-wrap items-center gap-3"><button onClick={openQrLink} className="flex h-11 items-center gap-2 rounded-xl bg-[#0e8065] px-5 text-sm font-bold text-white shadow-[0_10px_22px_rgba(14,128,101,0.2)] transition hover:bg-[#096a54] active:scale-[0.97]"><QrCode size={18} />ربط حساب واتساب</button><button onClick={() => setShowAssistantPanel(true)} className="flex h-11 items-center gap-1.5 rounded-xl border border-[#caa45a]/50 bg-[#caa45a]/10 px-3 text-sm font-bold text-[#e0bd6d] transition hover:bg-[#caa45a]/20">مساعد بسّام <Bot size={16} /></button><button onClick={() => setShowFlowPanel(true)} className="flex h-11 items-center gap-1.5 rounded-xl border border-[#b9d9cc] bg-white/60 px-3 text-sm font-bold text-[#245846] transition hover:bg-white">أنشئ قائمة خدمات <MessageCircle size={16} /></button><button onClick={() => setShowStatusPanel(true)} className="flex h-11 items-center gap-1.5 rounded-xl px-2 text-sm font-bold text-[#245846] transition hover:bg-white/60">الحالات <Eye size={16} /></button></div>
            </div>
      ...
      {showLinkPanel && <ClearLinkPanel linkMethod={linkMethod} setLinkMethod={setLinkMethod} session={session} isBusy={connectionBusy} error={connectionError} onStartQr={() => void startLink("qr")} onStartPairing={(phone) => void startLink("pairing", phone)} onClose={() => setShowLinkPanel(false)} />}
      ...
      {showAssistantPanel && <AssistantPanel settings={assistantSettings} isBusy={assistantBusy} error={assistantError} onSave={persistAssistantSettings} onClose={() => setShowAssistantPanel(false)} />}
    </div>
  );
}
...
/** Golden Protocol: explicit QR states replace a decorative placeholder with operational feedback. */
function ClearLinkPanel({ linkMethod, setLinkMethod, session, isBusy, error, onStartQr, onStartPairing, onClose }: { linkMethod: "qr" | "code"; setLinkMethod: (method: "qr" | "code") => void; session: SessionSnapshot; isBusy: boolean; error: string | null; onStartQr: () => void; onStartPairing: (phone: string) => void; onClose: () => void }) {
  const [phone, setPhone] = useState("");
  const qrReady = session.status === "awaiting_qr" && Boolean(session.qrDataUrl);
  const waitingForQr = isBusy || session.status === "connecting" || (session.status === "awaiting_qr" && !qrReady);
  const failure = error || session.error;
  const connected = session.status === "connected";
  const state = connected
    ? { title: "تم ربط حساب واتساب", detail: "الجلسة متصلة الآن وتستقبل الأحداث." }
    : qrReady
      ? { title: "رمز QR جاهز للمسح", detail: "افتح الأجهزة المرتبطة في واتساب وامسح الرمز الآن." }
      : waitingForQr
        ? { title: "جارٍ إنشاء رمز QR…", detail: "نتصل بعامل واتساب الآن. يستغرق هذا عادةً أقل من 10 ثوانٍ." }
        : failure
          ? { title: "تعذر إنشاء رمز QR", detail: "لم يكتمل الاتصال. راجع الرسالة أدناه ثم حاول مرة واحدة." }
          : { title: "لم يبدأ إنشاء الرمز بعد", detail: "اضغط الزر أدناه لبدء إنشاء رمز QR." };

  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#102e25]/45 p-0 backdrop-blur-[3px] sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-label="ربط حساب واتساب"><div className="w-full max-w-[840px] overflow-hidden rounded-t-[28px] border border-white/50 bg-[#fbfaf6] shadow-[0_30px_90px_rgba(18,51,42,0.28)] sm:rounded-[28px]"><div className="flex items-center justify-between border-b border-[#e1e3dc] px-5 py-4 sm:px-7"><div><p className="text-xs font-bold tracking-[0.08em] text-[#6f897e]">إعداد الجلسة</p><h2 className="mt-1 font-display text-xl font-bold tracking-[-0.04em] text-[#1c4337]">اربط حساب واتساب</h2></div><button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-xl text-[#62796f] hover:bg-[#eff1eb]" aria-label="إغلاق"><X size={19} /></button></div><div className="grid lg:grid-cols-[0.88fr_1.12fr]"><aside className="border-b border-[#e1e3dc] bg-[#f2f4ee] p-6 lg:border-b-0 lg:border-l lg:p-7"><div className="flex items-center gap-2"><span className="grid h-7 w-7 place-items-center rounded-lg bg-[#dceee4] text-[#0e8065]"><Hash size={15} /></span><p className="text-sm font-bold text-[#315347]">تسلسل الربط الواضح</p></div><ol className="mt-5 space-y-5">{["اضغط إنشاء رمز QR مرة واحدة.", "انتظر رسالة «رمز QR جاهز للمسح».", "امسح الرمز من الأجهزة المرتبطة في واتساب."].map((step, index) => <li key={step} className="flex items-start gap-3"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full border border-[#b9d5c6] bg-[#fbfaf6] font-mono text-[10px] font-bold text-[#0e8065]">{index + 1}</span><span className="pt-0.5 text-sm leading-6 text-[#60786d]">{step}</span></li>)}</ol><div className="mt-8 flex gap-2 rounded-xl border border-[#d6e7dd] bg-[#edf7f0] p-3 text-xs leading-5 text-[#39705e]"><ShieldCheck size={17} className="mt-0.5 shrink-0" />لا تحدّث الصفحة ولا تضغط الزر مرة ثانية أثناء إنشاء الرمز.</div></aside><div className="p-6 sm:p-7"><div className="flex rounded-xl bg-[#eef0eb] p-1"><button onClick={() => setLinkMethod("qr")} className={`flex-1 rounded-lg py-2 text-xs font-bold transition ${linkMethod === "qr" ? "bg-[#fbfaf6] text-[#0e8065] shadow-sm" : "text-[#70867b]"}`}>رمز QR</button><button onClick={() => setLinkMethod("code")} className={`flex-1 rounded-lg py-2 text-xs font-bold transition ${linkMethod === "code" ? "bg-[#fbfaf6] text-[#0e8065] shadow-sm" : "text-[#70867b]"}`}>كود الربط</button></div>{linkMethod === "qr" ? <div className="mt-6 flex flex-col items-center"><div className={`relative grid h-[190px] w-[190px] place-items-center overflow-hidden rounded-2xl border bg-white p-3 shadow-sm sm:h-[220px] sm:w-[220px] ${qrReady ? "border-[#0e8065]/40" : failure ? "border-[#e57e63]/40" : "border-[#d5dfd8]"}`}>{qrReady ? <img src={session.qrDataUrl ?? ""} alt="رمز QR لربط واتساب" className="h-full w-full object-contain" /> : <div className="flex max-w-[155px] flex-col items-center text-center"><span className={`mb-4 grid h-12 w-12 place-items-center rounded-2xl ${failure ? "bg-[#fff0ec] text-[#d96143]" : connected ? "bg-[#e2f1e9] text-[#0e8065]" : "bg-[#edf4ef] text-[#0e8065]"}`}>{waitingForQr ? <RefreshCw size={22} className="animate-spin" /> : connected ? <CircleCheck size={23} /> : failure ? <AlertTriangle size={22} /> : <QrCode size={22} />}</span><p className="text-sm font-bold leading-6 text-[#315347]">{state.title}</p><p className="mt-2 text-xs leading-5 text-[#71857b]">{state.detail}</p></div>}{waitingForQr && <span className="absolute inset-3 rounded-xl border border-[#0e8065]/20" />}</div><div className={`mt-4 w-full rounded-xl border p-3 text-right text-xs leading-5 ${failure ? "border-[#f0c5b9] bg-[#fff5f1] text-[#a44b33]" : qrReady || connected ? "border-[#cce4d7] bg-[#eef8f2] text-[#39705e]" : "border-[#d7e6dc] bg-[#f3f8f4] text-[#52766a]"}`} aria-live="polite"><div className="flex items-start gap-2">{waitingForQr ? <RefreshCw size={15} className="mt-0.5 shrink-0 animate-spin" /> : failure ? <AlertTriangle size={15} className="mt-0.5 shrink-0" /> : <Radio size={15} className="mt-0.5 shrink-0" />}<span>{state.detail}</span></div></div><button onClick={onStartQr} disabled={waitingForQr || qrReady || connected} className="mt-5 flex min-h-11 items-center gap-2 rounded-xl bg-[#0e8065] px-4 py-2 text-xs font-bold text-white shadow-[0_8px_18px_rgba(14,128,101,0.16)] transition hover:bg-[#096a54] disabled:cursor-not-allowed disabled:bg-[#789c8f] disabled:shadow-none">{connected ? "تم الربط بنجاح" : qrReady ? "الرمز جاهز للمسح" : waitingForQr ? "جارٍ إنشاء الرمز…" : failure ? "حاول إنشاء رمز جديد" : "إنشاء رمز QR الآن"}<RefreshCw size={14} className={waitingForQr ? "animate-spin" : ""} /></button></div> : <div className="mt-8"><label className="text-sm font-bold text-[#315347]" htmlFor="phone">رقم الهاتف</label><div className="mt-3 flex gap-2"><input id="phone" value={phone} onChange={(event) => setPhone(event.target.value)} dir="ltr" placeholder="+966 5X XXX XXXX" className="h-12 min-w-0 flex-1 rounded-xl border border-[#d7dfd8] bg-white px-4 text-sm outline-none placeholder:text-[#a7b2ab] focus:border-[#0e8065] focus:ring-4 focus:ring-[#0e8065]/10" /><button onClick={() => onStartPairing(phone)} disabled={isBusy || !phone.trim()} className="h-12 rounded-xl bg-[#0e8065] px-4 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-60">{isBusy ? "جارٍ إنشاء الكود…" : "إنشاء الكود"}</button></div><p className="mt-3 text-xs leading-5 text-[#74877e]">أدخل الرقم بصيغة دولية ثم افتح واتساب لإدخال كود الربط عند طلبه.</p><div className="mt-6 rounded-xl border border-dashed border-[#c5d8cc] bg-[#f5f8f4] p-4 text-center font-mono text-xl tracking-[0.25em] text-[#0e8065]">{session.pairingCode || "— — — — — — — —"}</div></div>}{failure && <p className="mt-4 rounded-xl border border-[#f0c5b9] bg-[#fff0ec] p-3 text-xs leading-5 text-[#a44b33]">{failure}</p>}<div className="mt-7 flex justify-between border-t border-[#e6e8e2] pt-5"><button onClick={onClose} className="text-xs font-bold text-[#647c71] hover:text-[#234e41]">إغلاق</button><span className="text-xs text-[#7b8f85]">حالة الاتصال تتحدث تلقائيًا.</span></div></div></div></div></div>;
}
